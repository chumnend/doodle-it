...
export function* authLoginSaga(action) {
  ...
  try {
    const url = `${import.meta.env.VITE_API_PREFIX}/v1/auth/login?apiKey=${import.meta.env.VITE_API_KEY}`;
...
export function* authRegisterSaga(action) {
  ...
  try {
    const url = `${import.meta.env.VITE_API_PREFIX}/v1/auth/register?apiKey=${import.meta.env.VITE_API_KEY}`;
    const res = yield axios.post(url, payload);
...
export function* authValidateSaga() {
  yield put(actions.authenticating());
